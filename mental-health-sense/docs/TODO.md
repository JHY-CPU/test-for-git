...
>
> 测试 **442 通过**；范围 1（链路正确性）**20/20**；范围 2（判别力）**9/9**。
>
...
> **已知未解决**（如实记录，非阻断）：
>
> - **永久性变化会无限期每日报警（优先级最高，仍未做）**：`PERM_step` 实测连续
>   91 天每天判 L3，冻结按 `max_freeze_days=14` 正常释放、阈值也回升
>   （1.18→1.91），但 score 4.5 远超其上，重新基线化追不上。而 L3 = 短信 +
>   网格员 + 强制响铃，且 `alert.py` 仍**无任何冷却/去重/抑制机制**。
>   检测层是对的，缺口在**预警策略层**：把“状态持续”当成了“每天都是新事件”。
>   见 VALIDATION §7.3。
>
>   **这条现在更要紧了**：抑郁通道是第二个会长期恒定输出同一结论的源
>   （群体基线对个体差异没有免疫力，表情少/语速慢/口音重的老人可能常年被判
>   同一等级）。两个“一直在响”的源比一个更快让家属关掉通知，而通知一关，
>   真正的新变化也收不到了。所以 `depression.alert` 目前刻意置 `false`——
>   **修好冷却之前不接第二个预警出口**。
>
>   正确的抽象是把“每日状态”换成“事件”：一段连续的 L2/L3 是一个事件，
>   只在开始 / 升级 / 缓解时通知，持续期给周期性回执（进周报、不响铃），
>   人工确认后转静默追踪。状态需持久化到 `data/logs/alert_state/{id}.json`
>   并原子写（每日批处理，内存态活不过今天）。
>
