...
> 📗 **第一次接触本项目，先读 `docs/OVERVIEW.md`**——不含代码的通俗讲解，
> 从"这系统到底在干什么"讲到每一条设计取舍的由来。本文档是技术总览，假定你已知道系统大致是什么。
...
**核心算法层已完工并验证**：517 个单元测试全部通过；范围 1（链路正确性）20/20、范围 2（判别力）9/9。每日趋势管道（读取 `data/raw/` → 双轨特征聚合 → 每轨独立 GRU 残差推理 → EWMA（偏离日冻结）→ 连续偏离判定 → 风险判定 → 预警 → 周报）已端到端打通，不依赖任何实时运行时。
...
2026-07-31 起共四轮走查（每轮数十条缺陷），完整实证、回归测试与验证状态变化
都在 `docs/VALIDATION.md` §8~§12。此处只留一句教训：

> **全绿不等于没问题，要看绿的是什么。** 多轮缺陷的共同特征是"验证脚本与单测
> 断言在错的层"——数的是 status 流转而非能否出等级、测的是辅助函数而非 rules
> 层行为、手搓的字典带着生产链路根本没写过的字段。它们能长期活在 312/332/481
> 全绿之下，正是因为**绿的不是那条路**。

### 系统验证（A/B/C 三层）

验证分三层，证据强度依次递增（完整方案与全部实证见 `docs/VALIDATION.md`）：

| 层次          | 回答的问题                      | 状态                                |
| ----------- | -------------------------- | --------------------------------- |
| **A 代码对不对** | 算法有没有被正确实现                 | ✅ 已完成（517 单测 + 端到端冒烟 20/20）       |
| **B 设计好不好** | 个人基线 / EWMA / 连续判定是否优于朴素替代 | 🚧 进行中（判别力 10 场景，当前 9/9，KM 场景不计分） |
| **C 真的有用吗** | 能否测出真实老人的心理下滑              | ⏳ 待真实数据（需公开数据集 + 临床金标准）           |

- "循环论证"缺口已**部分**打破：判别力脚本带真实噪声与混淆项；但
  `generate_simulation_data.py` 仍是循环的（异常方向=检测方向），只能测链路与信号隔离。
- 两个验证脚本均已固定随机种子（数据用 crc32、GRU 用 `torch.manual_seed`），连跑两次
  结果一致——曾因未固定 torch 种子给出**不可复现的 19/19**，掩盖了真实灵敏度缺口（缺陷⑥）。
- 📖 GRU 训练的完整细节（结构/样本/耗时/无验证集设计/建档期实验）见 `docs/TRAINING.md`。
...
#    若系统 PYTHONPATH 含 /opt/ros，pytest 会在收集阶段崩在 lark（一条都跑不到），
#    用 env -u PYTHONPATH python -m pytest 绕开（见下方「使用指南 → 运行测试」）。
python -m pytest
```
...
├── docs/                            # 文档
│   ├── README.md                    # 本文档（技术总览）
│   ├── OVERVIEW.md                  # 通俗讲解（面向不写代码的读者）
│   ├── FEATURES.md                  # 双轨特征字典（13 维逐项详解）
│   ├── TRAINING.md                  # GRU 训练详解
│   └── VALIDATION.md                # 三层验证框架 + 走查执行记录
│
├── data/                            # 数据目录
│   ├── raw/                         # 原始传感器数据（JSON）
│   │   ├── sleep/{id}/              # 小贝壳睡眠数据（{date}.json）
│   │   ├── activity/{id}/           # PIR + IPC活动数据（{date}.json）
│   │   └── camera/{id}/             # C6c 边缘人形检测（{date}.json，copresence_min 聚合来源）
...
│   ├── conftest.py                  # 共享 fixture（含 alert_state 目录隔离）
...
│   ├── test_depression_aggregate.py # 抑郁：多片段聚合
│   ├── test_depression_contract.py  # 抑郁：契约 schema 与校验
│   ├── test_depression_isolation.py # ★ 抑郁：四条隔离约束静态守卫
│   ├── test_depression_runner.py    # 抑郁：编排（subprocess 驱动）
│   ├── test_depression_status.py    # 抑郁：状态枚举与可展示白名单
│   ├── test_depression_store.py     # 抑郁：读写 + 有效期
...
│   ├── test_regression_2026_08_03.py # 第四轮走查 4 缺陷回归（10 条，VALIDATION §12）
│   ├── test_report_llm.py           # 周报正文 LLM（DeepSeek/OpenAI 兼容）与规则回落测试
...
> "连续 N 天"按**自然日**数，不按日志记录条数。

> 幅度门槛与方向闸门是 L2/L3 的两道附加闸：与提醒级共用同一条幅度门槛、门槛只算在
> "驱动本次升级的那段连续偏离"上、防止"睡眠全面好转"被判成严重风险——设计动机与
> 实测见 `docs/VALIDATION.md` §8.3 与缺陷⑤。
...
**AND 的条件越多，每个条件的门槛可以越低**：睡眠/节律为 1.5，
`social_decline` 为 **1.0**——三个必选维的变异系数差一个量级，同一个 ratio
对它们不是同一件事。降门槛的安全性由**结构**而非单维门槛提供（三项 AND +
"7 天滚动窗需 5 天"），`TN_short_social` 场景专守这条线，确认没有拿误报换灵敏度。
z 分被三重压缩的完整机理见 `docs/VALIDATION.md` 缺陷⑥。
...
（逐维定义、计算方式与权重见 `docs/FEATURES.md`）。
...
"连续 N 天"这句话看着简单，实现上有三个坑，都踩过（完整实证见
`docs/VALIDATION.md` §8.5）：

1. **按记录条数数 ≠ 按自然日数**：设备离线后断裂两端的偏离日会被当成连续日拼起来——
   实测 5 条日志跨 22 个日历日（断 17 天）仍数出 `consecutive=5` → L3。现在按自然日逐日回溯。
2. **"跳过"必须有上限**：缺日与降级日走同一条路径（都是"这天我们不知道"），既不累加
   也不打断；但连续跳过超过 `risk.continuity.max_skip_days`（默认 3）就打断——否则
   一次长时间离线又会把两段无关的偏离粘起来。
3. **加载窗口要够宽**：`circadian_disruption` 降级模式要求连续 7 天达标，若只加载 7 条
   日志，历史里一个降级日被跳过就会让计数上限掉到 6，规则**数学上永远不可激活**。
   加载天数现由 `rules.required_history_days()` 从各规则门槛推导（当前 25 天）。

> 等级判定窗口仍固定为最近 **7 个自然日**——"最近状况有多严重"只该看最近一周；
> 只有风险类型的持续性统计才回溯更远。
...
（训练的完整细节见 `docs/TRAINING.md`）。

### 1. 训练数据健康门禁（防"学坏"）

训练前用 **MAD（中位数绝对偏差）** 做逐特征离群筛查（`src/baseline/data_health.py`，
冷启动时调用）：个别离群天**剔除后训练**；离群天占比过高（默认 > 50%）**拒绝建档**、
提示顺延，而非用脏数据建立基线；Scaler 在清洗后的数据上拟合，归一化基准不被异常天带偏。
（机制详解见 `docs/TRAINING.md` §6.3。）

### 2. 微调防污染（只用正常天）

每周微调只用 `is_deviation=False` 的正常天（`weekly_retrain`），排除已判偏离的日期；
正常天不足则跳过微调，而非用脏数据。否则无脑用"最近 30 天"会把异常学进基线，让系统
越来越迟钝。（机制详解见 `docs/TRAINING.md` §6.4。）

### 3. 冷启动兜底（消除建档期盲区）

GRU 就绪前（建档期 35 天），用**中位数 / MAD 稳健基线**的加权 z-score 做基础离群检测
（`src/baseline/cold_start_fallback.py`），sigma 默认 **3.0** 比 GRU 轨更保守——建档期
滑动基线本身不稳，宁可漏报也不要一开始就误报动摇信任。GRU 一就绪，兜底自动停用。

> ⚠️ **这一层曾经形同虚设**：兜底确实算出了偏离，但 `judge` / `rules` 的状态白名单
> 只收 `("success","observation")`，**兜底轨的结果整个被丢掉**——建档期连续 6 天严重
> 恶化仍 `risk_level` 全 0，一条预警发不出。现在 `cold_start_fallback` 与
> `success`/`observation` 一起由 `src/utils/status.py` 统一定义为"可评估状态"，
> **加新状态只改那一个文件**。接线不是简单加白名单：兜底的分是稳健 |z|，与 GRU 轨的
> 残差尺度不可比，幅度门槛必须同时改成 severity 相对量。完整实证见
> `docs/VALIDATION.md` §8.2 / §8.3。

### 4. 过拟合抑制与可回滚

- **early-stopping + 回滚最优权重**：建档期滑窗后不到 30 个训练样本，150 epoch 易过拟合；
  连续 `patience` 轮 loss 无改善则提前停止并回滚到最优权重。冷启动与每周微调**共用同一
  训练循环** `_train_loop`（patience 20 / 10）。（机制详解见 `docs/TRAINING.md` §6.1。）
- **模型版本化**：每周微调覆盖 `gru.pth` 前先备份为 `gru.prev.pth`，微调把模型搞坏时可回滚。

---

## 特征设计（13 维）

13 个特征（睡眠轨 8 维 + 社交轨 5 维）聚焦两条可**非接触**监测的公认通路：
**自主神经失调**（睡眠 + HRV）与**行为退缩**（活动 + 社交）。抑郁相关的
精神运动迟滞通路已移除，改由外部专用抑郁模型负责（见「抑郁评估旁路通道」一节）。

- **逐维定义 / 计算方式 / 权重 / 异常方向见 `docs/FEATURES.md`**（特征字典），
  本处不再重复维护同一份表。
- **测量效度提示**：上述指标的**金标准是 PSG（多导睡眠图）**，本系统为非接触估算，
  设备能否复现 PSG 级别精度需设备验证实验。特征**选择**有据，特征**测量**精度需实测；
  `night_hr_mean` 是 `hrv_rmssd` 的**弱代理**（小贝壳拿不到逐拍间期，数学上算不出
  RMSSD），已如实降权到 1.0。
- **对话轮次已移除**：v2.0 的 `social_turns` 依赖拾音器 VAD，音频链路删除后，社会接触
  改由 C6c 边缘侧人形检测的 `copresence_min` 承担，代价是拿不到"是否在交谈"。

> ⚠️ **隐私边界（2026-07-31 重新划定，两件事必须分开说）：**
> 
> - **GRU 管线完全不使用音频**：社交轨 5 维一维未变，`copresence_min` 只来自边缘侧
>   人形检测的逐秒人数，视频不出户、只上报当日聚合分钟数。
> - **抑郁评估模块会录制短音视频片段**：落盘 `data/raw/video_clips/`，只在检测到
>   "有正脸 + 有连续语音"时导出。
> 
...
> - **EWMA**：按 `day_key` 去重（`day_key <= last_day_key` 拒收），重跑同一天不会把分
>   喂进基线两次、也不会让冻结计数多加一次；乱序补算历史日同样被挡住。
> - **阈值**：重跑复用首跑落盘的三个阈值，保证 `is_deviation` 判定幂等；但只在
>   **同一条产出路径**之间复用（兜底轨与 GRU 轨量纲不可比，见
>   `inference._load_prior_thresholds`）。
> - **预警事件**：通道游标 `last_processed_day` 跨事件保留，**早于它的历史日一律
>   不动状态、不通知**——此前事件一缓解游标被清空，补算一个历史高危日会真的推送
>   （实证见 `docs/VALIDATION.md` §11.1）。
...
> 两轨各自的列名与**顺序**即 `SLEEP_FEATURES` / `SOCIAL_FEATURES`
> （`src/baseline/scaler_utils.py`）——顺序即向量下标，必须与
> `config/feature_weights.json` 严格一致。每维的含义与来源见 `docs/FEATURES.md`。
...
实测 E001 60 天 12 次推送只对应 **4 个真实事件**；`PERM_step` 91 天连续 L3 发了
**91 次**（修复后 4 次）。

**后果不是"吵"，是系统最终变哑**：家属关掉通知后，真正的**新**变化也收不到了——
方向闸门、幅度门槛、持续性统计前面所有为压低误报做的工作，在通知被关掉的那一刻
全部失去出口。**检测层是对的**，缺口在预警策略层，所以修复没有碰
`judge.py` / `rules.py` 一行（完整修复记录见 `docs/VALIDATION.md` §10）。
...
- **事件边界**（隔多久没观测就算两段无关的事件）**按通道取**，由 `alert._event_gap_days`
  决定，刻意不给独立配置键：`baseline` 复用 `risk.continuity.max_skip_days`（默认 3），
  与 `walk_back_days` 的断段语义一致；`depression` 用 `depression.assessment.valid_days`
  ——那条通道是稀疏事件驱动的，几周才有一次合格片段，套用 3 天会让每次评估都判成
  新事件、去重完全失效。
- **状态持久化**：`data/logs/alert_state/{elder_id}.json`，原子写。必须落盘的理由与
  EWMA 的 `freeze_streak` 完全相同：**这是每日批处理，进程每天起一次就退，
  内存态活不过今天**。容错沿用 `ewma` 的四条：状态另存小文件 / 原子写 /
  缺文件按默认起算 / 损坏则重置该通道并继续（不抛、不连坐另一条流）。
...
两者**尺度不可比**，这与本项目"绝不跨轨比较绝对分"是同一条不变量——睡眠轨和
社交轨这两条自家的轨都不敢比，何况训练目标完全不同的外部模型。更要紧的是
**防正反馈**：若抑郁分能标记异常日，异常日会被排除出每周微调 → 个人基线越缩越窄
→ 更容易判偏离 → 偏离又被拿去佐证抑郁——系统**自己证明自己**。
（通俗版见 `docs/OVERVIEW.md`「抑郁评估：一条完全独立的旁路」。）
...
代码层的线已经接好（抑郁事件流独立、评估回正常发缓解、按通道取断段窗口），
事件映射也刻意保守——即使判"重度"也只到 L2、不惊动网格员——所以将来换到可用
checkpoint 后，只需把 `depression.alert` 改为 `true` 一个配置项即可启用整条预警链路。
在那之前，周报里那一节带**未校准警示**（"情绪状态评估（研究性，未校准）"），
对外口径仍是行为观察，不下诊断。

⚠️ **设备到货后必须做域验证**：真机位（客厅监控视角，而非访谈构图）下，这套
"有正脸 + 有连续语音"的片段截取流程还能不能截出合格样本——这是当前唯一可能
让整条抑郁线作废的风险，必须用真设备复验后再谈启用。

### 展示：为什么周报里要排"历次"时间线

大众基线对个体差异**没有免疫力**——天生表情少、语速慢的老人可能常年被判同一等级。
周报把历次评估排成序列看"变没变"而不是"高不高"，能把这种固定偏置降成背景噪音。
这只是展示层排序，**不涉及任何拟合、不产生个人基线**（样本太稀疏也根本估不出来）。
...
> 下面是关键段的节选，**不是完整文件**——`training.initial.build_days/holdout_days`、
> `risk.risk_rules`（三规则门槛）与整个 `depression:` 段等未在此列出。
> 完整键值以 `config/settings.yaml` 为准。
...
alert:                     # 由 alert.trigger_alert 读取，缺项回落内置默认
  ...
  events:                  # 事件抽象的行为参数（见「预警按事件去重」）
    notify_on_resolve: true       # 缓解也通知（要完全不发缓解通知，改这里）
    new_type_cooldown_days: 3     # 新风险类型的最短通知间隔
    weekly_receipt: true          # 持续期每周一条回执（进周报，不推送）
    resolve: { action: "push_notification", notify: ["children"] }
    # ⚠️ resolve.notify 是**基线**收件人，不是最终名单：事件曾达到过的最高等级
    #    的收件人会被并进来且不可删除（L3 缓解一定通知 community_worker）。
...
按定义 `severity > 1 ⟺ is_deviation`。**门槛必须 > 1**：取 1.0 等于恒真，
"防止擦线偏离仅凭连续天数升到 L3"这条防线会**静默失效**。

两个门槛取值不同也有原因：`sustained_severity`（1.15）只作用在**当前这段连续偏离**上；
`high_spike_severity`（1.5）作用在整个 7 天窗（含已恢复的日子），唯一职责是
"已经恢复了，但前几天那个尖峰仍值得保持关注"——bar 理应更高。
（实证见 `docs/VALIDATION.md` §8.3。）

## 消融实验设计

8 项消融实验的变量与预期结论见 `docs/VALIDATION.md` §3.2（本处不重复维护——
同一张表两处各写一份必然再次漂移）。
...
> 版本变更要点完整保留在 git 提交历史中，本文档不再维护"最后更新 / 版本要点"清单。
> 它容易过时，还曾与正文矛盾（v2.1.1 块里写过"预警冷却/去重仍未做"，与
> 「预警按事件去重」一节冲突）。要了解某次改动的动机，查对应提交信息。
