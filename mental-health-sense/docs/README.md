...
**核心算法层已完工并验证**：475 个单元测试全部通过；范围 1（链路正确性）20/20、范围 2（判别力）9/9。每日趋势管道（批处理：读取 `data/raw/` → 双轨特征聚合（睡眠 8 维 / 社交 5 维）→ 每轨独立 GRU 残差推理 → EWMA（偏离日冻结）→ 连续偏离判定 → 风险判定（含方向闸门，每轨各自算等级取较高者）→ 预警 → 周报）已端到端打通，不依赖任何实时运行时。
...
| 层次 | 回答的问题 | 状态 | 说明 |
|------|-----------|------|------|
| **A 代码对不对** | 算法有没有被正确实现 | ✅ 已完成 | 475 单测（22 文件）+ 端到端冒烟（`validate_synthetic.py` 20/20，含双轨信号隔离），每日管道链路已打通 |
...
│
├── tests/                           # 单元测试（22 个测试文件 / 475 用例，确定性可复现）
